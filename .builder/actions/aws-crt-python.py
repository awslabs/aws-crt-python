...
class AWSCrtPython(Builder.Action):
    def run(self, env):        
        ...
        actions = [
            ...
            ['sudo', venv_python, '-m', 'pip', 'install', '--upgrade', 'autopep8', 'boto3'],
            ['sudo', venv_python, '-m', 'pip', 'install', '.', '--install-option=--verbose', '--install-option=build_ext', '--install-option=--include-dirs{openssl_include}', '--install-option=--library-dirs{openssl_lib}'],
