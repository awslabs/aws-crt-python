...
class Mqtt5ClientTest(NativeResourceTest):
    ...
    def test_operation_shared_subscription(self):
        ...
        client_options_subscriber_1 = mqtt5.ClientOptions(
            host_name=input_host_name,
            port=8883,
            tls_ctx=io.ClientTlsContext(tls_ctx_options),
            connect_options=mqtt5.ConnectPacket(client_id=create_client_id())
        )
        client_options_subscriber_2 = mqtt5.ClientOptions(
            host_name=input_host_name,
            port=8883,
            tls_ctx=io.ClientTlsContext(tls_ctx_options),
            connect_options=mqtt5.ConnectPacket(client_id=create_client_id())
        )
        client_options_publisher = mqtt5.ClientOptions(
            host_name=input_host_name,
            port=8883,
            tls_ctx=io.ClientTlsContext(tls_ctx_options),
            connect_options=mqtt5.ConnectPacket(client_id=client_id_publisher)
        )

        callbacks_subscriber_1 = Mqtt5TestCallbacks()
        callbacks_subscriber_2 = Mqtt5TestCallbacks()
        callbacks_publisher = Mqtt5TestCallbacks()

        client_subscriber_1 = self._create_client(
            client_options=client_options_subscriber_1, callbacks=callbacks_subscriber_1)
        client_subscriber_2 = self._create_client(
        ...
        callbacks_subscriber_2.future_connection_success.result(TIMEOUT)
        client_publisher.start()
        callbacks_publisher.future_connection_success.result(TIMEOUT)

        topic_filter = "test/MQTT5_Binding_Python_" + client_id_publisher
        topic_filter_shared = "$share/crttest/test/MQTT5_Binding_Python_" + client_id_publisher
        payload = "Hello Subscriber"

        subscriptions = []
        subscriptions.append(mqtt5.Subscription(topic_filter=topic_filter_shared, qos=mqtt5.QoS.AT_LEAST_ONCE))
        subscriber_packet = mqtt5.SubscribePacket(subscriptions=subscriptions)
        subscribe_future_1 = client_subscriber_1.subscribe(subscribe_packet=subscriber_packet)
        suback_packet_1 = subscribe_future_1.result(TIMEOUT)
        self.assertIsInstance(suback_packet_1, mqtt5.SubackPacket)
        subscribe_future_2 = client_subscriber_2.subscribe(subscribe_packet=subscriber_packet)
        suback_packet_2 = subscribe_future_2.result(TIMEOUT)
        self.assertIsInstance(suback_packet_2, mqtt5.SubackPacket)

        # Expected publish count 10 shared between the two clients evenly
        callbacks_subscriber_1.on_publish_receive_expected = 5
        callbacks_subscriber_2.on_publish_receive_expected = 5
        publishes = 10
        for x in range(0, publishes):
            ...
            publish_future = client_publisher.publish(packet)
            publish_future.result(TIMEOUT)

        callbacks_subscriber_1.future_expected_publishes_received.result(TIMEOUT)
        callbacks_subscriber_2.future_expected_publishes_received.result(TIMEOUT)

        client_subscriber_1.stop()
        callbacks_subscriber_1.future_stopped.result(TIMEOUT)
        client_subscriber_2.stop()
        callbacks_subscriber_2.future_stopped.result(TIMEOUT)
        client_publisher.stop()
        callbacks_publisher.future_stopped.result(TIMEOUT)
